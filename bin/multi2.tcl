...
proc proc_sfpjar2_1 {router cmd {rate_expected 400}} {
    ...
    for {set i 0} {$i<$cout_llen} {incr i 1} {
        ...
        if {($matchok1==1) && ($matchok2==1)} {
            ...
            set rate_gap_perc [expr $rate_gap_perc * 100]

            myputs "$router:stream#$i:displayed rate:calculated rate $pps:$rate_calc ($rate_gap_perc/100 inconsistent)"
            myputs "  ($packets_now - $packets_prev) packets / ($time_now - $time_prev) seconds = ${rate_calc}pps"
            
            if { $rate_gap_perc >=0.5 } {
                myputs "found traffic loss in router $router:stream#$i: - rate $rate_gap diff than expected ($rate_expected)!"
