...
proc check_flag {list_of_router_and_cmd_list_flag} {
    ...
    foreach router [array names r_carray] {
        ...
        foreach cmd $r_carray($router) {
            ...
            set flag($router,$i) [$handler $router $cmd]
            incr i 1
            }
        }
    }
    #if 1st and 4th cmd one detected issue and the other is ok, then it's the issue
    #either vrf traffic not put into data-mdt , or the vice versa
    if { $flag($router,1) ^ $flag($router,4) } {
...
proc proc_sfpjar2_1 {router cmd {rate_expected 400}} {
    ...
    for {set i 0} {$i<$cout_llen} {incr i 1} {
        ...
        if {($matchok1==1) && ($matchok2==1)} {
            ...
            #display 0.3 as 30%
            myputs "$router:stream#$i:displayed rate:calculated rate $pps:$rate_calc ($rate_diff_with_pps diff)"
            myputs "  caculated rate:($packets_now - $packets_prev) packets / ($time_now - $time_prev) seconds = ${rate_calc}pps"
            
            if { $rate_diff_with_expected_ratio >= $rate_gap_ratio } {
                myputs "found traffic loss in router $router:stream#$i: - $rate_calc is $rate_diff_with_expected_perc% diff than expected ($rate_expected)!"
...
myputs "take a baseline from router:[array names check]..."
send_log "take a baseline from router:[array names check]...\n"
