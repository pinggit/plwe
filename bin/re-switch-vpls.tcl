...
proc myexpect {router pattern datasent {mytimeout 240}} {
...
proc persist_expect {router pattern datasent {mytimeout 240} {round 5}} {
    set i 1
    while {![myexpect $router $pattern $datasent $mytimeout]} {
        #myexpect $router ".*" ""
        incr i
...
proc switchover {router {hold_interval 300} {su_password herndon1}} {
    ...
    exp_send -i $session "\r"
    myexpect $router "> $" "request chassis routing-engine master switch" 60
...
proc restart_process {router process {su_password "herndon1"}} {
    ...
    myexpect $router "% $"        "su"
    myexpect $router "Password:$" "$su_password"
    myexpect $router "% $"        "uptime\r"
    #myexpect $router "% $"        "ps aux | grep rpd\r"
    myexpect $router "% $"        "ps aux | grep $process\r"
...
proc pre_work {router} {
    ...
    #myexpect $router "> $" "show bgp summary | match 10.64.50.111" 240
    myexpect $router "> $" "show bgp summary | match 192.168.1.81" 240
    set buf_bgp [myexpect $router "> $" "expect_out" 180]
...
proc action {router} {
    global debug session2host host2session
    set session $host2session($router)
    set captured_output [myexpect $router "> $" "show system uptime"]
    
    myexpect $router "> $" "configure"

    for {set i 601} {$i<=1100} {incr i 1} {
    #    myexpect $router "# $" "delete class-of-service interfaces ge-6/0/2 unit $i output-forwarding-class-map VPLS_PPCOS_class_map_10:20:30:10:10:20_out"
    #    myexpect $router "# $" "delete class-of-service interfaces ge-6/0/2 unit $i output-traffic-control-profile tcp-vlan-6-0-2-group-2"
    #    myexpect $router "# $" "delete class-of-service interfaces ge-6/0/2 unit $i output-traffic-control-profile shared-instance 100"
    #    myexpect $router "# $" "delete class-of-service interfaces ge-6/0/2 unit $i classifiers ieee-802.1 CE_VPLS_ieee_BA_CLASSIFIER_PPCOS_in"
        myexpect $router "# $" "deactivate interfaces ge-1/3/0 unit $i"
        myexpect $router "# $" "deactivate routing-instances 13979:333$i"
        myexpect $router "# $" ""
    }

        #myexpect $router "# $" "deactivate groups VPLS_CNTRL_WORD_ADD"
        #myexpect $router "# $" "delete apply-groups VPLS_CNTRL_WORD_ADD"
        #myexpect $router "# $" "load set vpls-no-tunnel-service.txt"
        myexpect $router "# $" "show | compare | last 40 | no-more"
        #myexpect $router "# $" "commit synchronize force"
        persist_expect $router "# $" "commit"
        #myexpect $router "# $" "rollback 1"
        #myexpect $router "# $" "show | compare | last 40 | no-more"
...
proc check {router} {
    ...
    myexpect $router "> $" ""
    myexpect $router "> $" {show interfaces ge-1/3/0 | match ge-1/3/0.[16-9] | count} 240
    set buf1 [myexpect $router "> $" "expect_out" 180]
    myputs "buf1:$buf1:500 lines:[regexp {500 lines} $buf1]"

    myexpect $router ".*" ""
    myexpect $router "> $" "show route forwarding-table extensive | match delete" 240
    set buf2 [myexpect $router "> $" "expect_out" 180]
...
#    }
	...
	myexpect $router $pattern $datasent $pattern_timeout	
...
for {set i 1} {$i<=$maxrounds} {incr i 1} {
    ...
    if {[info exists dead_time]} {
        if { $curr_time > $dead_time } {
            ...
            #myexpect $router "> $" ""
            #myexpect $router "> $" "configure"
            #myexpect $router "# $" "load override backup-b4-change.txt"
            #myexpect $router "# $" "commit"
            #myexpect $router "# $" "run request system reboot both-routing-engines"
            #myexpect $router "Reboot the system ? \[yes,no\] (no) yes" "yes"

            do_patterns_actionst $router recover
    ...
    myexpect $router ".*" ""
    myexpect $router "> $" "configure"
    myexpect $router "# $" "rollback 1" ;#toggle RIs
    myexpect $router "# $" "show | compare | last 40 | no-more"
    persist_expect $router "# $" "commit"
