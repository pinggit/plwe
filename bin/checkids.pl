...
#    my $count = shift;
#    my $count = shift;
    printf "%02x ", unpack( "C", substr($id, 0, 1) );
...
###    printf " : id size = %04x($self->{idSize})\n", $self->{idSize} ;
  } elsif( $self->{s1} == 0x5000 ){
    $self->{idSize} = 0 ;
    $self->{rsize} = $self->{n1} + $self->{idSize} + 1;
###    print "\n" ;
  } elsif( $self->{s1} == 0x4000 ){
    $self->{idSize} = 0 ;
    $self->{rsize} = $self->{n1} + $self->{idSize} ;
###    print "\n" ;
  } else {
    print "ERROR: non-30,40,50\n"
  }
#
#
...
###  $self->{unresCnt} ++ ;
  if( $self->{s1} == 0x3000 ){
    ...
    if(exists $cfgsToListIndex{$cfgName}) {
   print "Listing Index of $cfgName\n" ;
    }
    for( my $i = 16; $i < $self->{DSz} ; $i += $self->{rsize} ){
      $id = substr( $self->{dBuf}, $i, $idSize+1  );
      if(exists $cfgsToListIndex{$cfgName}){
     printIndex($id);
      }
# conitnue if first byte(flag) is 0x7f (deleted)
# value is expected to be zeros 
      next if ord($id) == 0x7f ;
      if( exists $ids{$id} ) {
     $ids{$id}++ ;
     $oops{$id} = $ids{$id};
    ...
    @ks = keys( %oops );
    if( $#ks != -1 ){
        print "!!!!!!!!!!!!! $cfgName : " . ($#ks+1) . " duplicate ids.\n" ; 
        for my $id (sort keys %oops) {
       printIndex($id, $oops{$id});
#       for( my $ii = 0 ; $ii < length $id ; $ii++ ){
#      printf "%02x", unpack( "C", substr( $id, $ii, 1 ) ) ;
#       }
#       print " - " . ($oops{$id} +1) . "  entries\n" ;
        }  
    } 
##    if
##%dups{   for( my $ii = 0 ; $ii < $idSize+1 ; $ii++ ){
##          printf "%02x", unpack( "C", substr( $id, $ii, 1 ) ) ;
##        }
##   print "\n" ;
##      }
...
#      printf "%02x", unpack( "C", substr( $self->{dBuf}, $i, 1 );
###      my $ff = unpack( "C", substr( $self->{dBuf}, $i, 1 ) );
###      printDel($ff) ;
###      for( my $j = 1 ; $j < 40 ; $j += 4){
###        printf " %08x", unpack( "N", substr( $self->{dBuf}, ($j + $i), 4 ) );
###      }
###      print "\n" ;
###    }
###  } else {
###    for my $i ( 12 .. ($self->{DSz} - 1) ){
#      printf "%02x", unpack( "C", substr( $self->{dBuf}, $i, 1 );
###      my $ff = unpack( "C", substr( $self->{dBuf}, $i, 1 ) );
###      printDel($ff) ;
###      last if $i >= 32 ;
