...
def mymain():   #{{{1}}}
    ...
    pat_compl_tag=re.compile((r'(<.*>)'))



    #read file into list{{{2}}}
    fd=open(fn,'r')
    ...
    for eachline in lines:       #{{{2}}}
        ...
        if debug: print 'get a line: ', linenum, '"', eachline, '"'
        ...
        if ioline is not None:
            ...
            #compl_tag=re.search(re.escape(pat4),
            compl_tag=pat_compl_tag.search(afterio)

            #complete tag {{{4}}}
            if compl_tag is not None:
                if debug: 
                ...
                merged=False
                #save in list lines2
                lines2.append(str(linenum).rjust(6)+' '+eachline)
